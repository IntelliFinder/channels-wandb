...
class EGNN_vel(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_edge_nf, hidden_node_nf, hidden_coord_nf,
                 ...
                 update_vel=False, color_steps=2, ef_dim=3, mixed=False, shared_wl=False):
        ...
        self.init_color_mixed_first = TwoFDisInit(ef_dim=ef_dim , k_tuple_dim=hidden_nf, activation_fn=act_fn)
        self.init_color_mixed = TwoFDisInit(ef_dim=ef_dim*(num_vectors)**2 , k_tuple_dim=hidden_nf, activation_fn=act_fn)
        # interaction layers
